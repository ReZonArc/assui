...
export class DevToolsHooks {
  ...
  private constructor() {
    ...
    if (!window.__ASSISTANT_UI_DEVTOOLS_HOOK__) {
      ...
      console.log("[Assistant UI] DevTools hook initialized", window.__ASSISTANT_UI_DEVTOOLS_HOOK__);
    } else {
      console.log("[Assistant UI] DevTools hook already exists", window.__ASSISTANT_UI_DEVTOOLS_HOOK__);
    }
  }
  ...
  static getInstance(): DevToolsHooks {
    if (!DevToolsHooks.instance) {
      ...
      // Expose instance methods to the hook for extension access
      if (typeof window !== "undefined") {
        const hook = (window as any).__ASSISTANT_UI_DEVTOOLS_HOOK__;
        if (hook) {
          hook.getInstance = () => DevToolsHooks.instance;
        }
      }
    }
  ...
  register(api: Partial<AssistantApi>): Unsubscribe {
    ...
    console.log("[Assistant UI] Registering API with ID", apiId, "Hook APIs size:", hook.apis.size);
    ...
    if (api.on) {
      try {
        entry.unsubscribe = api.on("*", (eventData: unknown) => {
          // The event data structure is { event: string, payload: any }
          const eventName = (eventData as any).event || "unknown";
          const payload = (eventData as any).payload || eventData;
